...
if __name__ == "__main__":
    ...
    for platform, test_url in test_urls.items():
        ...
        try:
            if platform == "douyin":
                ...
                # content_text = video_info.get("content_text")

                ai_result = openai_service.get_ai_assistant_text( trace_id, content_text, {})
    ...
    #             PROMPTS = {
    #                 "core": f"请你化身**顶尖爆款视频策划人**，以制造刷屏级内容的敏锐嗅觉，审视并提炼出**[{content_text}]**中最具冲击力、最能引发用户共鸣和传播的核心观点/价值点。请用简洁精炼的语言，分点列出，并简要阐述每个观点**为何具备成为爆款的潜质**（例如：情感触发点、争议性、实用价值、新奇度、反差感等）。",
    #                 "formula": f"请你扮演一位**深谙传播之道的爆款视频操盘手**，对**[{content_text}]**进行深度解剖，提炼总结出其中可被复用、可迁移的**“爆款密码”或“增长范式”**。请清晰阐述这个“范式”的关键组成部分（如：钩子设计、情绪曲线、价值点呈现节奏、互动引导策略、记忆点打造技巧等），并说明**如何将其巧妙应用于其他内容的创作中**，以显著提升引爆流行、实现增长的可能性。",
    #                 "copywriting": f"""请你以**深谙小红书平台特性与用户心理的资深内容运营专家**身份，围绕这段文字**[{content_text}]**，创作一篇**至少100字**、**极具“网感”和“种草力”**的小红书爆款笔记文案。要求：
    # 1.  **开头3秒吸睛**，瞬间抓住用户注意力。
    ...
    #                 "golden3s": f"""请你作为**精通“黄金三秒”法则、能瞬间点燃用户好奇心的爆款视频大师**，针对这段文字**[{content_text}]**，构思**3-5个**能够在**视频开篇3秒内**就**牢牢锁住观众眼球、激发强烈观看欲望**的**创意开场方案**。请具体描述每个方案的：
    # * **核心悬念/钩子**
